...
function LeadForm({ subject, dark=false }: { subject:string; dark?:boolean }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { router.push("/thank-you") } else setLoading(false)
    ...
    <div dir="rtl" style={{direction:"rtl"}}>
      ...
      <section className="ph-hero-section" style={{position:"relative",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
        ...
        <nav className="ph-nav" style={{
          ...
          <div className="ph-nav-links" style={{display:"flex",gap:32,alignItems:"center"}}>
            ...
            <a href={`tel:${PHONE}`} onClick={() => (window as any).trackCall && (window as any).trackCall()} style={{fontSize:"0.82rem",fontWeight:700,color:scrolled?DARK:"white",textDecoration:"none"}} dir="ltr">01117322733</a>
        ...
        <div className="ph-hero-grid" style={{
          ...
          <div className="ph-hero-form slide-up" style={{
            ...
            <div className="ph-hero-form-btns" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
              <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بمشاريع Palm Hills الساحلية")}`} onClick={() => (window as any).trackWhatsapp && (window as any).trackWhatsapp()} target="_blank" rel="noopener noreferrer"
                style={{padding:"11px",background:"#25D366",color:"white",fontWeight:700,fontSize:"0.72rem",textAlign:"center",textDecoration:"none"}}>
                💬 واتساب
              </a>
              <a href={`tel:${PHONE}`} onClick={() => (window as any).trackCall && (window as any).trackCall()}
      ...
      <section id="contact" className="ph-contact-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",minHeight:"65vh"}}>
        <div className="ph-contact-red" style={{padding:"80px 60px",background:RED,display:"flex",flexDirection:"column",justifyContent:"center"}}>
          ...
          <a href={`tel:${PHONE}`} onClick={() => (window as any).trackCall && (window as any).trackCall()} style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"2rem",fontWeight:300,color:"white",textDecoration:"none",display:"block",marginBottom:32,transition:"opacity .2s"}}
            ...
            <div style={{padding:"24px 28px"}}>
              {popupSent ? (
                ...
                <form onSubmit={submitPopup}>
                  ...
                  <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بالحجز في أرض ديزني من Palm Hills — الساحل الشمالي")}`} onClick={() => (window as any).trackWhatsapp && (window as any).trackWhatsapp()} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:40,display:"grid",gridTemplateColumns:"1fr 1fr"}}>
        <a href={`tel:${PHONE}`} onClick={() => (window as any).trackCall && (window as any).trackCall()} style={{padding:"16px",background:RED,color:"white",fontWeight:700,fontSize:"0.78rem",textAlign:"center",textDecoration:"none"}}>📞 اتصل الآن</a>
        <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بمشاريع Palm Hills الساحلية")}`} onClick={() => (window as any).trackWhatsapp && (window as any).trackWhatsapp()} target="_blank" rel="noopener noreferrer"
